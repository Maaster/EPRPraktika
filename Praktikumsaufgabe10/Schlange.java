...
public class Schlange {
     ...
     */
    private int bewegungsRichtung = 0;
    ...
    public boolean geheVoran(Punkt futterPos) {
        ...
        {
            case 0:
                if(futterPos.istGleich(new Punkt(schlangenPos[0].gibX(), schlangenPos[0].gibY() - 1))) {
                    schlangeVerlaengern(new Punkt(0, -1));
                }
                else {
                    for (int i = 0; i < schlangenPos.length - 1; i = i + 1) {
                    if(schlangenPos[i].istGleich(new Punkt(schlangenPos[0].gibX(), schlangenPos[0].gibY() - 1))) {
                        kannZiehen = false;
                    }
                }
            ...
            case 90:
                if(futterPos.istGleich(new Punkt(schlangenPos[0].gibX() + 1, schlangenPos[0].gibY()))) {
                    schlangeVerlaengern(new Punkt(1,0));
                }
                else {
                    for (int i = 0; i < schlangenPos.length - 1; i = i + 1) {
                    if(schlangenPos[i].istGleich(new Punkt(schlangenPos[0].gibX() + 1, schlangenPos[0].gibY()))) {
                        kannZiehen = false;
                    }
                }
